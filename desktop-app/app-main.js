...
const TB = global.teamBoard || { version: app.getVersion(), update: null, updDir: () => path.join(app.getPath("documents"), "Team Board", "updates"), cmp: () => 0 };
...
function menu() {
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([
    ...
    { label: "Help", submenu: [
      ...
      { type: "separator" },
      { label: "Install update from file…", click: installUpdate },
      ...(TB.update ? [{ label: "Go back to the previous version", click: undoUpdate }] : []),
      { label: "About Team Board", click: () => dialog.showMessageBox(win, { type: "info", title: "Team Board", message: `Team Board ${TB.version}`, detail: `Data folder:\n${dataDir}${TB.update ? `\n\nRunning update ${TB.version} (installed version ${TB.builtin}).` : ""}` }) } ] }
  ]));
}

async function installUpdate() {
  const r = await dialog.showOpenDialog(win, { title: "Choose the Team Board update file", filters: [{ name: "Team Board update", extensions: ["asar"] }], properties: ["openFile"] });
  if (r.canceled || !r.filePaths[0]) return;
  const src = r.filePaths[0];
  let pkg; try { pkg = JSON.parse(fs.readFileSync(path.join(src, "package.json"), "utf8")); } catch (e) { pkg = null; }
  if (!pkg || pkg.name !== "team-board" || !fs.existsSync(path.join(src, "app-main.js"))) return dialog.showMessageBox(win, { type: "error", message: "That isn't a Team Board update file." });
  if (TB.cmp(pkg.version, TB.version) <= 0) return dialog.showMessageBox(win, { type: "info", message: `You already have version ${TB.version}. This file is version ${pkg.version}.` });
  try {
    const dir = TB.updDir(); fs.mkdirSync(dir, { recursive: true });
    fs.copyFileSync(src, path.join(dir, `${pkg.version}.asar`));
    try { const f = path.join(dir, "skipped.json"); const bad = JSON.parse(fs.readFileSync(f, "utf8")).filter(x => x !== `${pkg.version}.asar`); fs.writeFileSync(f, JSON.stringify(bad)); } catch (e) {}
  } catch (e) { return dialog.showMessageBox(win, { type: "error", message: "Couldn't install the update.", detail: e.message }); }
  srv.log.info(`Update ${pkg.version} installed`);
  const a = await dialog.showMessageBox(win, { type: "info", message: `Version ${pkg.version} is installed.`, detail: "Restart Team Board to use it. Your data isn't touched.", buttons: ["Restart now", "Later"], defaultId: 0 });
  if (a.response === 0) { srv.flush(); app.relaunch(); app.exit(0); }
}
async function undoUpdate() {
  const a = await dialog.showMessageBox(win, { type: "question", message: `Go back from version ${TB.version}?`, detail: "The update is set aside and Team Board restarts. Your data isn't touched.", buttons: ["Go back", "Cancel"], defaultId: 1, cancelId: 1 });
  if (a.response !== 0) return;
  try { const f = path.join(TB.updDir(), "skipped.json"); let bad = []; try { bad = JSON.parse(fs.readFileSync(f, "utf8")); } catch (e) {} bad.push(TB.update.file); fs.writeFileSync(f, JSON.stringify(bad)); } catch (e) {}
  srv.flush(); app.relaunch(); app.exit(0);
}
app.whenReady().then(async () => {
  ...
  srv.log.info(`Program version ${TB.version}${TB.update ? " (from update file)" : ""}`);
  menu();
  createWindow();
  ...
  if (TB.failedUpdate) { srv.log.error("Update failed to start: " + TB.failedUpdate); dialog.showMessageBox(win, { type: "warning", title: "Update skipped", message: `An update couldn't start, so it was set aside. Team Board is running version ${TB.version}.`, detail: TB.failedUpdate }); }
